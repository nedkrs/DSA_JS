...
// B (2nd node in A and 3rd node in B) are different node references.
...
export default function IntersectionOfTwoLinkedLists(
    ...
    let bigger = len1 > len2 ? headA : headB;
    let smaller = len1 > len2 ? headB : headA;

    while (diff > 0) {
        bigger = bigger?.next || null;
        diff--;
    }
