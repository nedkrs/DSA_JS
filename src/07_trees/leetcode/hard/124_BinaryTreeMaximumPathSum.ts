...
export default function BinaryTreeMaximumPathSum(root: Node<number> | null) {
    let max = -Infinity;

    function pathSum(node: Node<number> | null) {
        if (!node) return 0;

        let leftGain = Math.max(pathSum(node?.left || null), 0);
        let rightGain = Math.max(pathSum(node?.right || null), 0);
