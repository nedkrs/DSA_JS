...
export default function HouseRobberTopDown(nums: number[]): number {
...
function rob(nums: number[], dp: number[], n: number): number {
    if (dp[n] !== undefined) return dp[n];

    dp[n] = Math.max(rob(nums, dp, n - 1), rob(nums, dp, n - 2) + nums[n]);
